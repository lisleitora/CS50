...
SELECT pl.first_name
  , pl.last_name
  , s.salary / pe.H AS dollars_per_hit
FROM players AS pl
JOIN salaries AS s ON (s.player_id = pl.id)
JOIN performances AS pe ON (
    pe.player_id = pl.id
    AND pe.year = s.year
  ) -- how dare you to jump off me 5 seconds after you came, you just wanted me for the cookies huh? you look at me with those (literal) puppy eyes... cute dog imma give ya more food. you greedy pup. why you must be so cute?
